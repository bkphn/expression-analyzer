...
def plot_single_class_cm(TP, TN, FP, FN, class_name):
    ...
    plt.tight_layout()
    #plt.savefig(os.path.join(FIG_DIR, f'cm_{class_name}.png'), dpi=300)
    plt.show()
...
def run_metrics(y_true, y_pred, class_names):
    ...
    metrics = {accuracy: 'Accuracy', sensitivity_recall: 'Recall',
               precision: 'Precision', f1_score: 'F1 Score', specificity: 'Specificity'}
...
def plot_metrics_table(table_data, class_names):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(FIG_DIR, 'metrics_table_rf.png'), dpi=300, bbox_inches='tight')
    plt.show()
